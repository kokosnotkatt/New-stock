...
const SignupPage = ({ onClose, onSwitchToLogin }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name) newErrors.name = t('signup.errors.nameRequired'); // (ควรเพิ่ม key)
    else if (formData.name.length < 2) newErrors.name = t('signup.errors.nameLength');
    ...
    else if (formData.password.length < 8) newErrors.password = t('signup.errors.passwordLength');
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      alert(t('signup.alert.success')); // (ควรเพิ่ม key)
      onClose?.();
    }, 1500);
  };
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center z-50 p-4 overflow-y-auto backdrop-blur-sm">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 left-10 w-72 h-72 bg-emerald-200 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-blob animation-delay-2000"></div>
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-md my-8 relative border border-white/20">
        ...
        <div className="p-8 pb-6">
          ...
          <p className="text-gray-600">{t('signup.joinUs')}</p>
        </div>
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-4">
          {/* Name Field */}
          <div>
            <label htmlFor="name" className="block text-sm font-semibold text-gray-700 mb-2">{t('signup.nameLabel')}</label>
          ...
          <button type="submit" disabled={isLoading}
            className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-3.5 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-green-500/30 mt-6"
          >
            {isLoading ? (
